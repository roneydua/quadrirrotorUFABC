...
#error                                                                         \
    "Due to API change, this sketch is compatible with STM32_CORE_VERSION  >= 0x01090000"
#endif
...
#define IBUSBM_NOTIMER -1 // no timer interrupt used
  void begin(HardwareSerial &serial, int8_t timerid = 0, int8_t rxPin = -1,
             int8_t txPin = -1);
#endif
  ...
  addSensor(uint8_t type,
            uint8_t len =
                2); // add sensor type and data length (2 or 4), returns address
  void setSensorMeasurement(uint8_t adr, int32_t value);

  void loop(void); // used internally for interrupt handline, but needs to be
                   // defined as public
  ...
  ControlUnion remoteControl;

private:
  enum State { GET_LENGTH, GET_DATA, GET_CHKSUML, GET_CHKSUMH, DISCARD };

  static const uint8_t PROTOCOL_LENGTH = 0x20;
  static const uint8_t PROTOCOL_OVERHEAD =
      3; // packet is <len><cmd><data....><chkl><chkh>, overhead=cmd+chk bytes
  static const uint8_t PROTOCOL_TIMEGAP =
      3; // Packets are received very ~7ms so use ~half that for the gap
  static const uint8_t PROTOCOL_CHANNELS = 14;
  //   static const uint8_t PROTOCOL_CHANNELS = 6;
  static const uint8_t PROTOCOL_COMMAND40 =
      0x40; // Command to set servo or motor speed is always 0x40
  static const uint8_t PROTOCOL_COMMAND_DISCOVER =
      0x80; // Command discover sensor (lowest 4 bits are sensor)
  static const uint8_t PROTOCOL_COMMAND_TYPE =
      0x90; // Command discover sensor (lowest 4 bits are sensor)
  static const uint8_t PROTOCOL_COMMAND_VALUE =
      0xA0; // Command send sensor data (lowest 4 bits are sensor)
  ...
  //   remoteControl. channel[PROTOCOL_CHANNELS]; // servo data received
  ...
  IBusBM *IBusBMnext = NULL; // pointer to the next class instance to be used
                             // to call the loop() method from timer interrupt
};
