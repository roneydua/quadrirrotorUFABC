 ...
 * Extended to also handle sensors/telemetry data to be sent back to the
 * transmitter, interrupts driven and other features.
 *
...
// pointer to the first class instance to be used to call the loop() method from
// timer interrupt will be initiated by class constructor, then daisy channed to
// other class instances if we have more than one
IBusBM *IBusBMfirst = NULL;

// Interrupt on timer0 - called every 1 ms
// we call the IBusSensor.loop() here, so we are certain we respond to sensor
// requests in a timely matter
#ifdef ARDUINO_ARCH_AVR
SIGNAL(TIMER0_COMPA_vect) {
  if (IBusBMfirst)
    IBusBMfirst->loop(); // gets new servo values if available and process any
                         // sensor data
}
#else
void onTimer() {
...
/*
 *  supports max 14 channels in this lib (with messagelength of 0x20 there is
 room for 14 channels) Example set of bytes coming over the iBUS line for
 setting servos: 20 40 DB 5 DC 5 54 5 DC 5 E8 3 D0 7 D2 5 E8 3 DC 5 DC 5 DC 5 DC
 5 DC 5 DC 5 DA F3 Explanation Protocol length: 20 Command code: 40 Channel 0:
 DB 5  -> value 0x5DB Channel 1: DC 5  -> value 0x5Dc Channel 2: 54 5  -> value
 0x554 Channel 3: DC 5  -> value 0x5DC Channel 4: E8 3  -> value 0x3E8 Channel
 5: D0 7  -> value 0x7D0 Channel 6: D2 5  -> value 0x5D2 Channel 7: E8 3  ->
 value 0x3E8 Channel 8: DC 5  -> value 0x5DC Channel 9: DC 5  -> value 0x5DC
    Channel 10: DC 5 -> value 0x5DC
    Channel 11: DC 5 -> value 0x5DC
    Channel 12: DC 5 -> value 0x5DC
    Channel 13: DC 5 -> value 0x5DC
    Checksum: DA F3 -> calculated by adding up all previous bytes, total must be
 FFFF
 */

#if defined(_VARIANT_ARDUINO_STM32_)
void IBusBM::begin(HardwareSerial &serial, TIM_TypeDef *timerid, int8_t rxPin,
                   int8_t txPin) {
#else
void IBusBM::begin(HardwareSerial &serial, int8_t timerid, int8_t rxPin,
                   int8_t txPin) {
#endif
  ...
  // we need to process the iBUS sensor protocol handler frequently enough (at
  // least once each ms) to ensure the response data from the sensor is sent on
  // time to the receiver if timerid==IBUSBM_NOTIMER the user is responsible for
  // calling the loop function
  this->IBusBMnext = IBusBMfirst;

  if (!IBusBMfirst && timerid != IBUSBM_NOTIMER) {
#ifdef ARDUINO_ARCH_AVR
    // on AVR architectures Timer0 is already used for millis() - we'll just
    // interrupt somewhere in the middle and call the TIMER0_COMPA_vect
    // interrupt
    OCR0A = 0xAF;
    TIMSK0 |= _BV(OCIE0A);
#else
// on other architectures we need to use a time
    ...
    timer = timerBegin(
        timerid, F_CPU / 1000000L,
        true); // defaults to timer_id = 0; divider=80 (1 ms); countUp = true;
    ...
    NRF_TIMER4->INTENSET = TIMER_INTENSET_COMPARE0_Enabled
...
#else
// It should not be too difficult to support additional architectures as most
// have timer functions, but I only tested AVR and ESP32
#warning                                                                       \
    "Timing only supportted for AVR, ESP32 and STM32 architectures. Use timerid IBUSBM_NOTIMER"
#endif
#endif
  }
  IBusBMfirst = this;
...
// called from timer interrupt or mannually by user (if IBUSBM_NOTIMER set in
// begin())
void IBusBM::loop(void) {

  // if we have multiple instances of IBusBM, we (recursively) call the other
  // instances loop() function
  if (IBusBMnext)
    IBusBMnext->loop();
      ...
      if (chksum == (v << 8) + lchksum) {
        ...
        if (buffer[0] == PROTOCOL_COMMAND40) {
          ...
          for (uint8_t i = 1; i < PROTOCOL_CHANNELS * 2 + 1; i += 2) {
            remoteControl.channel[i / 2] = buffer[i] | (buffer[i + 1] << 8);
          }
          cnt_rec++;
        } else if (adr <= NumberSensors && adr > 0 && len == 1) {

          // all sensor data commands go here
          // we only process the len==1 commands (=message length is 4 bytes
          // incl overhead) to prevent the case the return messages from the
          // UART TX port loop back to the RX port and are processed again. This
          // is extra precaution as it will also be prevented by the
          // PROTOCOL_TIMEGAP required
          ...
          case PROTOCOL_COMMAND_TYPE: // 0x90, send sensor type
            ...
            chksum = 0xFFFF - (0x06 + PROTOCOL_COMMAND_TYPE + adr +
                               s->sensorType + s->sensorLength);
            break;
...
uint16_t IBusBM::readChannel(uint8_t channelNr) {
  if (channelNr < PROTOCOL_CHANNELS) {
    return remoteControl.channel[channelNr];
  } else {
    return 0;
  }
}
